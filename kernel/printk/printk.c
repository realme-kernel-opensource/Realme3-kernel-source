...
/*xing.xiong@BSP.Kernel.Driver, 2018/12/22, Add for uart log of release version*/
	if (printk_force_uart){
		pr_err("printk_force_uart is true");
		return true;
	}
#endif
	pr_err("printk_disable_uart = %d\n", printk_disable_uart);
...
{
#ifdef VENDOR_EDIT
	/* Weiguixiong@RM.BSP.Stability.Debug, 2018/12/20, temp add for uart suspend panic */
        dump_stack();
        pr_err("mt_need_uart_console = %d, printk_disable_uart = %d, printk_force_uart = %d\n", mt_need_uart_console,printk_disable_uart,printk_force_uart);
#endif

#ifdef VENDOR_EDIT
/*xing.xiong@BSP.Kernel.Debug, 2018/11/22, Add for forcing to enable uart */
	if (printk_force_uart) {
		/*weiguixiong@RM.BSP.Kernel.Debug, 2018/12/13, Avoid panic about force enable uart*/
...
static size_t print_prefix(const struct printk_log *msg, bool syslog, char *buf)
{
	...
	/*Wei.guixiong@RM.BSP.Kernel.Debug, 2019/1/7, Add for forcing to enable uart */
#ifdef VENDOR_EDIT 
...
/*Wei.guixiong@RM.BSP.Kernel.Debug, 2019/1/7, Add for forcing to enable uart */
#ifdef VENDOR_EDIT
		if (((printk_disable_uart == 1) && (printk_force_uart == 0)) && (con->flags & CON_CONSDEV))
#else
