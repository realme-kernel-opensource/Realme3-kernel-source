		...
		for(j = 0, index1 = 0; j <= ilitek_ini_file_data[i].iKeyValueLen; j++) {
			if(ilitek_ini_file_data[i].pKeyValue[j] == ',' || ilitek_ini_file_data[i].pKeyValue[j] == ';' ||
				...
				if(record != '.') {
					...
					data[(count % 4)] = temp;
					if ((count/4) >= (FRAME_NODE)) {
						ipio_info("count/4 = %d\n", count/4);
						ipio_info("ilitek_ini_file_data[i].pSectionName = %s desp = %s\n", ilitek_ini_file_data[i].pSectionName, desp);
						break;
					}
					if ((count % 4) == 3) {
						if (data[0] == 1) {
...
int core_parser_get_tdf_value(int index, char *str)
{
	...
	for (token = strsep(&s, "."); token != NULL; token = strsep(&s, ".")) {
		...
		point++;
		if (point >= 2 && (strncmp(token, "0", strlen("0")) == 0)) {
			mul10 = false;
			ipio_info("mul10 = %d\n", mul10);
		}
		if (point >= sizeof(tmp))
...
int core_parser_path(char *path)
{
	...
	tmp = vmalloc((int)(fw_ini->size) + 1);
	if (ERR_ALLOC_MEM(tmp)) {
		ipio_err("Failed to allocate tmp memory, %ld\n", PTR_ERR(tmp));
		res = -ENOMEM;
		goto out;
	}
	memset(tmp, 0, (fw_ini->size) + 1);
	/* change all characters to lower case */
	for (i = 0; i < (int)fw_ini->size; i++)
		tmp[i] = tolower(fw_ini->data[i]);

	res = get_ini_phy_data((uint8_t *)tmp,(int)fw_ini->size);
	if (res < 0) {
