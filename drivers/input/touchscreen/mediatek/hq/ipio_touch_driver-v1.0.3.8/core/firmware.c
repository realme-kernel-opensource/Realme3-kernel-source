...
int check_hex_crc(void)
{
	...
	if ((addr_len >= 4) && (addr_len < MAX_AP_FIRMWARE_SIZE)) {
		...
		check_fail = true;
		ipio_info("get ap addr len error addr_len = 0x%X\n", addr_len);
	}
	ipio_debug(DEBUG_FIRMWARE, "ap block, driver crc = 0x%X, hex ap crc = 0x%X\n", ap_crc, hex_ap_crc);
	...
	if ((addr_len >= 4) && (addr_len < MAX_DLM_FIRMWARE_SIZE)) {
		...
		check_fail = true;
		ipio_info("get dlm addr len error addr_len = 0x%X\n", addr_len);
	}
	ipio_debug(DEBUG_FIRMWARE, "dlm block, driver crc = 0x%X, hex dlm crc = 0x%X\n", dlm_crc, hex_dlm_crc);
	...
	if ((addr_len >= 4) && (addr_len < MAX_MP_FIRMWARE_SIZE)) {
		...
		check_fail = true;
		ipio_info("get mp addr len error addr_len = 0x%X\n", addr_len);
	}
	ipio_debug(DEBUG_FIRMWARE, "mp block, driver crc = 0x%x, hex mp crc = 0x%x\n", mp_crc, hex_mp_crc);
	...
	if ((addr_len >= 4) && (addr_len < MAX_GESTURE_FIRMWARE_SIZE)) {
		...
		check_fail = true;
		ipio_info("get gesture addr len error addr_len = 0x%X\n", addr_len);
	}
	ipio_debug(DEBUG_FIRMWARE, "gesture block, driver crc = 0x%x, hex gesture crc = 0x%x\n", gesture_crc, hex_gesture_crc);

	if ((check_fail) || (ap_crc != hex_ap_crc) || (mp_crc != hex_mp_crc)\
		|| (dlm_crc != hex_dlm_crc) || (gesture_crc != hex_gesture_crc)) {
		ipio_info("crc erro use header file data check_fail = %d\n", check_fail);
		ipio_info("ap block, driver crc = 0x%X, hex ap crc = 0x%X\n", ap_crc, hex_ap_crc);
		ipio_info("dlm block, driver crc = 0x%X, hex dlm crc = 0x%X\n", dlm_crc, hex_dlm_crc);
		ipio_info("mp block, driver crc = 0x%x, hex mp crc = 0x%x\n", mp_crc, hex_mp_crc);
		ipio_info("gesture block, driver crc = 0x%x, hex gesture crc = 0x%x\n", gesture_crc, hex_gesture_crc);
		return -1;
	}
	else {
		ipio_debug(DEBUG_FIRMWARE, "check file crc ok\n");
		return 0;
	}
}
...
int core_firmware_get_h_file_data(void)
{
	...
	if (ERR_ALLOC_MEM(flash_fw)) {
		ipio_info("Failed to allocate flash_fw memory, %ld\n", PTR_ERR(flash_fw));
		res = -ENOMEM;
		goto out;
	}
	...
	for (i = 0; i < (ipd->ili_fw_size-64); i++) {
	...
	ipio_info("core_firmware->core_version = 0x%X\n", core_firmware->core_version);
	...
	ipio_info("MP_HEX_ADDRESS + MAX_MP_FIRMWARE_SIZE = 0x%X\n", MP_HEX_ADDRESS + MAX_MP_FIRMWARE_SIZE);
    memcpy(mp_fw, flash_fw + MP_HEX_ADDRESS, MAX_MP_FIRMWARE_SIZE);
	ipio_info("core_gesture->start_addr + core_gesture->length = 0x%X\n", core_gesture->start_addr + MAX_MP_FIRMWARE_SIZE);
	memcpy(gesture_fw, flash_fw + core_gesture->start_addr, core_gesture->length);

	/* Extract block info */
	if (CTPM_FW[32]) {
		for (i = 0; i < 6; i++) {
			if (((CTPM_FW[32] >> i) & 0x01) == 0x01) {
				if ((i + 1) == 6) {
					g_flash_block_info[block].start_addr = (CTPM_FW[0] << 16) + (CTPM_FW[1] << 8) + (CTPM_FW[2]);
					g_flash_block_info[block].end_addr = (CTPM_FW[3] << 16) + (CTPM_FW[4] << 8) + (CTPM_FW[5]);
				} else {
					g_flash_block_info[block].start_addr = (CTPM_FW[34 + i * 6] << 16) + (CTPM_FW[35 + i * 6] << 8) + (CTPM_FW[36 + i * 6]);
					g_flash_block_info[block].end_addr = (CTPM_FW[37 + i * 6] << 16) + (CTPM_FW[38 + i * 6] << 8) + (CTPM_FW[39 + i * 6]);
				}
		...
		g_flash_block_info[2].start_addr = 0x13000;
		g_flash_block_info[2].end_addr = 0x1cfff;
		g_flash_block_info[3].start_addr = 0x1d000;
		g_flash_block_info[3].end_addr = 0x1efff;
	} else {
		block = CTPM_FW[33];
	    for (i = 0; i < 4; i++) {
...
int host_download_getapcode(void)
{
	int res = 0, ap_crc, ap_dma, dlm_crc, dlm_dma, method;
	uint8_t *read_ap_buf = NULL, *read_dlm_buf = NULL;
	int retry = 10;
	int i = 0;
	uint32_t reg_data = 0;
	res = core_config_ice_mode_enable();
	...
	method = method & 0xff;
	ipio_info("0 method of calculation for crc = %x\n", method);
	...
	if (method >= CORE_TYPE_E) {
		...
		read_download(0, MAX_AP_FIRMWARE_SIZE, read_ap_buf, SPI_UPGRADE_LEN);
		read_download(DLM_START_ADDRESS, MAX_DLM_FIRMWARE_SIZE, read_dlm_buf, SPI_UPGRADE_LEN);
		for (i = 0; i < MAX_AP_FIRMWARE_SIZE; i++) {
			if (read_ap_buf[i] != ap_fw[i]) {
				ipio_info("ap iram check fail i = 0x%X, ap = 0x%X read 0x%X", i, ap_fw[i], read_ap_buf[i]);
			}
		}
		for (i = 0; i < MAX_DLM_FIRMWARE_SIZE; i++) {
			if (read_dlm_buf[i] != dlm_fw[i]) {
				ipio_info("dlm iram check fail i = 0x%X, dlm = 0x%X read 0x%X", i, dlm_fw[i], read_dlm_buf[i]);
			}
		}
		if (memcmp(ap_fw, read_ap_buf, MAX_AP_FIRMWARE_SIZE) != 0) {
			ipio_info("Check AP data: FAIL\n");
			res = UPDATE_FAIL;
			//goto out;
		} else {
			ipio_info("Check AP data: SUCCESS\n");
		}
		if (memcmp(dlm_fw, read_dlm_buf, MAX_DLM_FIRMWARE_SIZE) != 0) {
			ipio_info("Check DLM data: FAIL\n");
			res = -2;
			goto out;
		} else {
			ipio_info("Check DLM data: SUCCESS\n");
		}
	} else {
		read_download(0, MAX_AP_FIRMWARE_SIZE, read_ap_buf, SPI_UPGRADE_LEN);
		read_download(DLM_START_ADDRESS, MAX_DLM_FIRMWARE_SIZE, read_dlm_buf, SPI_UPGRADE_LEN);
...
upgrade_fail:
...
#else
	int res = 0, ap_crc, ap_dma, dlm_crc, dlm_dma, method;
	uint8_t *read_ap_buf = NULL, *read_dlm_buf = NULL, *read_mp_buf = NULL;
	...
	res = check_hex_crc();
	if (res < 0) {
		ipio_info("crc erro use header file data\n");
		if (core_firmware_get_h_file_data() < 0) {
			ipio_info("Failed to get h file data\n");
		}
        res = 0;
	}
	method = core_config_ice_mode_read(core_config->pid_addr);
	...
	ipio_info("core_gesture->entry = %d core_gesture->ap_length = 0x%X\n", core_gesture->entry, core_gesture->ap_length);
	memset(read_gesture_buf, 0xFF, core_gesture->ap_length);
	...
	if (core_fr->actual_fw_mode == protocol->test_mode) {
		...
		if (mode) {
			...
			/* write hex to the addr of Gesture code */
			ipio_info("Writing data into Gesture code ...\n");
			ipio_info("gesture crc 0x%X 0x%X 0x%X 0x%X\n", gesture_fw[MAX_GESTURE_FIRMWARE_SIZE - 4], gesture_fw[MAX_GESTURE_FIRMWARE_SIZE - 3], gesture_fw[MAX_GESTURE_FIRMWARE_SIZE - 2], gesture_fw[MAX_GESTURE_FIRMWARE_SIZE - 1]);
			if (write_download(core_gesture->ap_start_addr, core_gesture->length, gesture_fw, SPI_UPGRADE_LEN) < 0) {
				ipio_err("SPI Write Gesture code data error\n");
			}
			ipio_info("read data into Gesture code ...\n");
			if (read_download(core_gesture->ap_start_addr, core_gesture->length, read_gesture_buf, SPI_UPGRADE_LEN)) {
				ipio_err("SPI Read Gesture code data error\n");
			}
			ipio_info("read gesture crc 0x%X 0x%X 0x%X 0x%X\n", read_gesture_buf[MAX_GESTURE_FIRMWARE_SIZE - 4], read_gesture_buf[MAX_GESTURE_FIRMWARE_SIZE - 3], read_gesture_buf[MAX_GESTURE_FIRMWARE_SIZE - 2], read_gesture_buf[MAX_GESTURE_FIRMWARE_SIZE - 1]);
...
static int convert_hex_file(uint8_t *pBuf, uint32_t nSize, bool isIRAM)
{
	...
	/* Parsing HEX file */
	if (pBuf[0] != ':') {
		ipio_err("Invalid hex format pbuf[0] = %c\n", pBuf[0]);
		goto out;
	}
	for (; i < nSize;) {
		int32_t nOffset;
...
int core_firmware_upgrade(const char *pFilePath, bool isIRAM)
{
	...
	if ((core_firmware->fw.data && (core_firmware->fw.size != 0))) {
		ipio_info("fw from image file,go to convert_hex!\n");
		goto convert_hex;
	}
	else {
		if (core_firmware->fw.data == NULL) {
			ipio_info("core_firmware_upgrade before request_firmwre\n");
			if(core_config->wr_update_type == 1){
				core_config->wr_update_type = 0;
			ret = request_firmware(&ili_fw_entry, ipd->fw_bin_name, &(ipd->spi->dev));
			}else{ 
				if (ipd->not_cts_test) {
					ret = request_firmware_select(&ili_fw_entry, ipd->fw_bin_sign_name, &(ipd->spi->dev));
				} else {
					ipio_info("cts test request_firmwre not use signed FW\n");
					ret = request_firmware(&ili_fw_entry, ipd->fw_bin_name, &(ipd->spi->dev));
				}
			}
			if (ret != 0) {
			...
			core_firmware->fw.data = vmalloc(ili_fw_entry->size);
			core_firmware->fw.size = ili_fw_entry->size;
			//copy bin fw to data buffer
			memcpy((u8 *)core_firmware->fw.data, (u8 *)(ili_fw_entry->data), ili_fw_entry->size);
			if (0 == memcmp((u8 *)core_firmware->fw.data, (u8 *)(ili_fw_entry->data), ili_fw_entry->size)) {
	...
	if ((!ERR_ALLOC_MEM(core_firmware->fw.data)) && (core_firmware->fw.size != 0) && (ipd->common_reset == 1)) {
		ipio_info("fw from image file\n");
		goto convert_hex;
	}
pFile:
