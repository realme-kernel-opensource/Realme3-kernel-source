...
#define ITO_TEST_CSV_FILE "/sdcard/ito"

#define SHORT_TEST_CSV_FILE "/sdcard/ito/ShortTest.csv"
#define OPEN_TEST_CSV_FILE "/sdcard/ito/OpenTest.csv"
#define FW_RAWDATA_CSV_FILE "/sdcard/ito/FWMutualTest.csv"
#define FW_CC_CSV_FILE "/sdcard/ito/FWCCTest.csv"
#define NOISE_TEST_CSV_FILE "/sdcard/ito/NoiseTest.csv"
#if LPGW_TEST
#define LPWG_RAWDATA_CSV_FILE "/sdcard/LPWGRawdataTest.csv"
#define LPWG_NOISE_CSV_FILE "/sdcard/LPWGNoiseTest.csv"
#endif

#if NVT_MP_CUSTOMIZED_LOG
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible, struct seq_file *m);
#if LPGW_TEST
int32_t nvt_mp_parse_lpwg_dt(struct device_node *root, const char *node_compatible, struct seq_file *m);
#endif
...
static void nvt_print_data_log_in_one_line(int32_t *data, int32_t data_num, struct seq_file *m)
{
	char *tmp_log = NULL;
	int32_t i = 0;
...
return:
	n.a.
*******************************************************/
static void nvt_print_lmt_array(char *s, int32_t *array, int32_t x_ch, int32_t y_ch, struct seq_file *m)
{
	int32_t j = 0;
	if(m == NULL) {
		printk("%s:\n", s);
		for (j = 0; j < y_ch; j++) {
			nvt_print_data_log_in_one_line(array + j * x_ch, x_ch, NULL);
			printk("\n");
		}

#if TOUCH_KEY_NUM > 0
		nvt_print_data_log_in_one_line(array + y_ch * x_ch, Key_Channel, NULL);
		printk("\n");
		...
		for (j = 0; j < y_ch; j++) {
			seq_printf(m, "[%d]\t", j);
			nvt_print_data_log_in_one_line(array + j * x_ch, x_ch, m);
			seq_printf(m, "\n");
		}

#if TOUCH_KEY_NUM > 0
		seq_printf(m, "[%d]\t", j);
		nvt_print_data_log_in_one_line(array + y_ch * x_ch, Key_Channel, m);
		seq_printf(m, "\n");
#endif /* #if TOUCH_KEY_NUM > 0 */
	}
}

void nvt_print_criteria(struct seq_file *m)
{
	NVT_LOG("++\n");

	if (ts->carrier_system) {
		//---PS_Config_Lmt_Short_Diff---
		nvt_print_lmt_array("PS_Config_Lmt_Short_Diff_P", PS_Config_Lmt_Short_Diff_P, X_Channel, Y_Channel, m);
		nvt_print_lmt_array("PS_Config_Lmt_Short_Diff_N", PS_Config_Lmt_Short_Diff_N, X_Channel, Y_Channel, m);
		//---PS_Config_Lmt_Short_Base---
		nvt_print_lmt_array("PS_Config_Lmt_Short_Base_P", PS_Config_Lmt_Short_Base_P, X_Channel, Y_Channel, m);
		nvt_print_lmt_array("PS_Config_Lmt_Short_Base_N", PS_Config_Lmt_Short_Base_N, X_Channel, Y_Channel, m);
	} else {
		//---PS_Config_Lmt_Short_Rawdata---
		nvt_print_lmt_array("PS_Config_Lmt_Short_Rawdata_P", PS_Config_Lmt_Short_Rawdata_P, X_Channel, Y_Channel, m);
		nvt_print_lmt_array("PS_Config_Lmt_Short_Rawdata_N", PS_Config_Lmt_Short_Rawdata_N, X_Channel, Y_Channel, m);
	}

	//---PS_Config_Lmt_Open_Rawdata---
	nvt_print_lmt_array("PS_Config_Lmt_Open_Rawdata_P", PS_Config_Lmt_Open_Rawdata_P, X_Channel, Y_Channel, m);
	nvt_print_lmt_array("PS_Config_Lmt_Open_Rawdata_N", PS_Config_Lmt_Open_Rawdata_N, X_Channel, Y_Channel, m);

	//---PS_Config_Lmt_FW_Rawdata---
	nvt_print_lmt_array("PS_Config_Lmt_FW_Rawdata_P", PS_Config_Lmt_FW_Rawdata_P, X_Channel, Y_Channel, m);
	nvt_print_lmt_array("PS_Config_Lmt_FW_Rawdata_N", PS_Config_Lmt_FW_Rawdata_N, X_Channel, Y_Channel, m);

	if (ts->carrier_system) {
		//---PS_Config_Lmt_FW_CC_I---
		nvt_print_lmt_array("PS_Config_Lmt_FW_CC_I_P", PS_Config_Lmt_FW_CC_I_P, X_Channel, Y_Channel, m);
		nvt_print_lmt_array("PS_Config_Lmt_FW_CC_I_N", PS_Config_Lmt_FW_CC_I_N, X_Channel, Y_Channel, m);
		//---PS_Config_Lmt_FW_CC_Q---
		nvt_print_lmt_array("PS_Config_Lmt_FW_CC_Q_P", PS_Config_Lmt_FW_CC_Q_P, X_Channel, Y_Channel, m);
		nvt_print_lmt_array("PS_Config_Lmt_FW_CC_Q_N", PS_Config_Lmt_FW_CC_Q_N, X_Channel, Y_Channel, m);
	} else {
		//---PS_Config_Lmt_FW_CC---
		nvt_print_lmt_array("PS_Config_Lmt_FW_CC_P", PS_Config_Lmt_FW_CC_P, X_Channel, Y_Channel, m);
		nvt_print_lmt_array("PS_Config_Lmt_FW_CC_N", PS_Config_Lmt_FW_CC_N, X_Channel, Y_Channel, m);
	}

	//---PS_Config_Lmt_FW_Diff---
	nvt_print_lmt_array("PS_Config_Lmt_FW_Diff_P", PS_Config_Lmt_FW_Diff_P, X_Channel, Y_Channel, m);
	nvt_print_lmt_array("PS_Config_Lmt_FW_Diff_N", PS_Config_Lmt_FW_Diff_N, X_Channel, Y_Channel, m);

#if LPGW_TEST
	//---PS_Config_Lmt_LPWG_Rawdata---
	nvt_print_lmt_array("PS_Config_Lmt_LPWG_Rawdata_P", PS_Config_Lmt_LPWG_Rawdata_P, X_Channel, Y_Channel, m);
	nvt_print_lmt_array("PS_Config_Lmt_LPWG_Rawdata_N", PS_Config_Lmt_LPWG_Rawdata_N, X_Channel, Y_Channel, m);
	//---PS_Config_Lmt_LPWG_Diff---
	nvt_print_lmt_array("PS_Config_Lmt_LPWG_Diff_P", PS_Config_Lmt_LPWG_Diff_P, X_Channel, Y_Channel, m);
	nvt_print_lmt_array("PS_Config_Lmt_LPWG_Diff_N", PS_Config_Lmt_LPWG_Diff_N, X_Channel, Y_Channel, m);
#endif
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch, uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	for (y = 0; y < y_ch; y++) {
		...
		nvt_print_data_log_in_one_line(rawdata + y * x_ch, x_ch, NULL);
		printk("\n");
	...
	nvt_print_data_log_in_one_line(rawdata + y_ch * x_ch, Key_Channel, NULL);
	printk("\n");
	...
	switch (TestResult) {
		...
		case -1:
			...
			for (i = 0; i < y_len; i++) {
				...
				if (!nvt_mp_test_result_printed)
					nvt_print_data_log_in_one_line(rawdata + i * x_len, x_len, NULL);
				nvt_mp_seq_printf(m, "\n");
			}
#if TOUCH_KEY_NUM > 0
			for (k = 0; k < Key_Channel; k++) {
			...
			if (!nvt_mp_test_result_printed)
				nvt_print_data_log_in_one_line(rawdata + y_len * x_len, Key_Channel, NULL);
			nvt_mp_seq_printf(m, "\n");
#endif /* #if TOUCH_KEY_NUM > 0 */
			break;
	}
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		...
		if (nvt_mp_parse_dt(np, mpcriteria, NULL)) {
			//---Download Normal FW---
...
struct file_operations nvt_selftest_fops = {
	.owner = THIS_MODULE,
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
static ssize_t nvt_lpwg_selftest_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	struct device_node *np = ts->client->dev.of_node;
	int retry=20;
	int len =0;
	...
	if ( *ppos ) {
	    	NVT_LOG("is already read the file\n");
	    	return 0;
	}
  	  *ppos += count;
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		...
		if (nvt_mp_parse_lpwg_dt(np, mpcriteria, NULL)) {
			//---Download Normal FW---
			nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME_SIGNED);
			mutex_unlock(&ts->lock);
			NVT_ERR("mp parse device tree failed!\n");
	...
	if ((TestResult_FWMutual == 0)
		&& (TestResult_FW_DiffMax == 0)) {
		sprintf(ts->gesture_test.message, "0 error(s). All test passed");
	} else {
		sprintf(ts->gesture_test.message, "2 error(s)");
	}
...
struct file_operations nvt_lpwg_selftest_fops = {
	.owner = THIS_MODULE,
	.read = nvt_lpwg_selftest_read,
...
return:
	n.a.
*******************************************************/
int32_t nvt_mp_parse_u32(struct device_node *np, const char *name, int32_t *para, struct seq_file *m)
{
	int32_t ret;
...
int32_t nvt_mp_parse_array(struct device_node *np, const char *name, int32_t *array,
		int32_t size, struct seq_file *m)
{
	struct property *data;
	int32_t len, ret;
#if NVT_DEBUG
	...
	if ((!data) || (!len) || (len < size)) {
		...
		if(m == NULL){
			...
			for (j = 0; j < Y_Channel; j++) {
				nvt_print_data_log_in_one_line(array + j * X_Channel, X_Channel, NULL);
				printk("\n");
			}
#if TOUCH_KEY_NUM > 0
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible, struct seq_file *m)
{
	...
	/* MP Config*/
	if (nvt_mp_parse_u32(np, "IC_X_CFG_SIZE", &IC_X_CFG_SIZE, m))
		return -1;

	if (nvt_mp_parse_u32(np, "IC_Y_CFG_SIZE", &IC_Y_CFG_SIZE, m))
		return -1;

#if TOUCH_KEY_NUM > 0
	if (nvt_mp_parse_u32(np, "IC_KEY_CFG_SIZE", &IC_KEY_CFG_SIZE, m))
		return -1;
#endif

	if (nvt_mp_parse_u32(np, "X_Channel", &X_Channel, m))
		return -1;

	if (nvt_mp_parse_u32(np, "Y_Channel", &Y_Channel, m))
		return -1;
	...
	if (ts->carrier_system) {
		if (nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Diff_P", PS_Config_Lmt_Short_Diff_P,
				X_Channel * Y_Channel + Key_Channel, m))
			return -1;
	...
	if (nvt_mp_parse_u32(np, "PS_Config_Diff_Test_Frame", &PS_Config_Diff_Test_Frame, m))
		return -1;
...
int32_t nvt_mp_parse_lpwg_dt(struct device_node *root, const char *node_compatible, struct seq_file *m)
{
	...
	/* MP Config*/
	if (nvt_mp_parse_u32(np, "IC_X_CFG_SIZE", &IC_X_CFG_SIZE,NULL))
		return -1;

	if (nvt_mp_parse_u32(np, "IC_Y_CFG_SIZE", &IC_Y_CFG_SIZE, NULL))
		return -1;

#if TOUCH_KEY_NUM > 0
	if (nvt_mp_parse_u32(np, "IC_KEY_CFG_SIZE", &IC_KEY_CFG_SIZE, NULL))
		return -1;
#endif

	if (nvt_mp_parse_u32(np, "X_Channel", &X_Channel, NULL))
		return -1;

	if (nvt_mp_parse_u32(np, "Y_Channel", &Y_Channel, NULL))
		return -1;
...
int32_t nvt_mp_proc_init(void)
{
	...
	NVT_proc_lpwg_selftest_entry =
		proc_create("nvt_lpwg_selftest", 0664, NULL, &nvt_lpwg_selftest_fops);
#endif
