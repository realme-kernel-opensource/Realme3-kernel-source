...
int check_fw(const struct firmware *fw)
{
	int ret =0;
	if(fw == NULL) {
		NVT_ERR("fw is NULL\n");
...
static int32_t update_firmware_request(char *filename)
{
	...
	while (1) {
		...
		if(!(strcmp(filename,BOOT_UPDATE_FIRMWARE_NAME))) {
			if(check_fw(fw_entry_normal)==0) {
				NVT_LOG("no need request normal fw\n");
				fw_entry = fw_entry_normal;
				goto BIN_PARSER;
			}
			NVT_LOG("it's need request normal fw\n");
			...
			fw_entry = fw_entry_normal;
		}else if(!(strcmp(filename,BOOT_UPDATE_FIRMWARE_NAME_SIGNED))){
			if(check_fw(fw_entry_normal)==0) {
				NVT_LOG("no need request normal fw\n");
				fw_entry = fw_entry_normal;
				goto BIN_PARSER;
			}
			NVT_LOG("it's need request_select normal fw\n");
			ret = request_firmware_select(&fw_entry_normal, filename, &ts->client->dev);
			if (ret != 0) {
				NVT_LOG("request_select firmware failed, get from headfile\n");
				request_fw_headfile = kzalloc(sizeof(struct firmware), GFP_KERNEL);
				if(request_fw_headfile == NULL) {
					NVT_LOG("request_fw_headfile kzalloc failed!\n");
					ret = -1;
			...
			if(check_fw(fw_entry_mp)==0) {
				NVT_LOG("no need request mp fw\n");
				fw_entry = fw_entry_mp;
				goto BIN_PARSER;
			}
			NVT_LOG("it's need request mp fw\n");
			...
			if (ret != 0) {
				...
				fw_entry_mp = request_fw_headfile;
				fw_entry = fw_entry_mp;
			}else
				fw_entry = fw_entry_mp;
		}
