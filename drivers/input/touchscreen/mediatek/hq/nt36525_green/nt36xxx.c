...
//static uint8_t bTouchIsAwake = 0;	//replaced by ts->bTouchIsAwake
...
int32_t nvt_read_pid(void)
{
	...
	//sprintf(ts->nvt_ts_info.manufacture, "0x%04X", ts->nvt_pid);
	//ts->nvt_ts_info.fw_path = BOOT_UPDATE_FIRMWARE_NAME_SIGNED;
	//NVT_LOG("firmware path fw_path=%s\n",ts->nvt_ts_info.fw_path);
...
info_retry:
	...
	sprintf(ts->nvt_ts_info.version, "EA6010%02X-1", ts->fw_ver);
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ts->reset_gpio = of_get_named_gpio_flags(np, "touch,reset-gpio", 0, &ts->reset_flags);
...
#if WAKEUP_GESTURE
	if (ts->bTouchIsAwake == NVT_TP_WKG) {
...
/*
	...
	printk("\n");
*/
//	printk("%02X %02X %02X %02X %02X %02X  ", point_data[1], point_data[2], point_data[3], point_data[4], point_data[5], point_data[6]);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	if ((ts->boot_mode == META_BOOT|| ts->boot_mode == FACTORY_BOOT )) {
		NVT_LOG("boot_mode is FACTORY or META mode not need to use tp\n");
		return -1;
	}
	...
	// need 20ms delay after POR(power on reset)
	msleep(20);
...
//Alloc fw_name/devinfo memory space
    ts->nvt_ts_info.fw_path = kzalloc(MAX_FW_NAME_LENGTH, GFP_KERNEL);
    if (ts->nvt_ts_info.fw_path == NULL) {
        NVT_LOG("panel_data.fw_name kzalloc error\n");
        kfree(ts->nvt_ts_info.fw_path);
		goto err_int_request_failed;
    }else{
		ts->nvt_ts_info.fw_path = BOOT_UPDATE_FIRMWARE_NAME_SIGNED;
	}

    ts->nvt_ts_info.version = kzalloc(MAX_DEVICE_VERSION_LENGTH, GFP_KERNEL);
    if (ts->nvt_ts_info.version == NULL) {
        NVT_LOG("manufacture_info.version kzalloc error\n");
        kfree(ts->nvt_ts_info.version);
		goto err_int_request_failed;
    }else{
		memcpy(ts->nvt_ts_info.version, "0x0", 3);//default version
	}

    ts->nvt_ts_info.manufacture = kzalloc(MAX_DEVICE_MANU_LENGTH, GFP_KERNEL);
    if (ts->nvt_ts_info.manufacture == NULL) {
        NVT_LOG("panel_data.fw_name kzalloc error\n");
        kfree(ts->nvt_ts_info.manufacture);
		goto err_int_request_failed;
    }else{
		ts->nvt_ts_info.manufacture = "novatek";
	}
	//nvt_get_fw_info();
/* chenyunrui@RM.BSP.TP.FUNCTION, 2018/11/8, Add end */
...
#if BOOT_UPDATE_FIRMWARE
	if(is_cts_test){
		NVT_LOG("cts test:in probe use Boot_Update_Firmware to update firmware!\n");
	...
	ts->bTouchIsAwake = NVT_TP_ACTIVE;
	NVT_LOG("end\n");
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if ((ts->bTouchIsAwake == NVT_TP_SLEEP) || (ts->bTouchIsAwake == NVT_TP_WKG)) {
		NVT_LOG("Touch is already suspend, status %d\n", ts->bTouchIsAwake);
		mutex_unlock(&ts->lock);
		return 0;
	}
	NVT_LOG("start\n");
	...
	if (ts->gesture_enable) {
		...
		ts->bTouchIsAwake = NVT_TP_WKG;
		ts->bTouchSuspendReason = NVT_TP_WKG;
		...
		ts->bTouchIsAwake = NVT_TP_SLEEP;
		ts->bTouchSuspendReason = NVT_TP_SLEEP;
...
#if WAKEUP_GESTURE
	if (ts->bTouchSuspendReason == NVT_TP_SLEEP) {
		nvt_irq_enable(true);
	}
#else
	nvt_irq_enable(true);
...
{
	if (ts->bTouchIsAwake == NVT_TP_ACTIVE) {
		NVT_LOG("Touch is already resume, status %d\n", ts->bTouchIsAwake);
		return 0;
	}
	NVT_LOG("start\n");
	ts->bTouchIsAwake = NVT_TP_ACTIVE;
	msleep(10); // add delay between mipi video on and backlight led on  bugid:1655606  liyan 20181127
	NVT_LOG("end\n");
	return 0;
...
{
	int32_t ret = 0,ret1=0,ret2=0,ret3=0;
	/* YunRui.Chen@RM.BSP.TP.FUNCTION, 2018/12/7, Add for cts gsi test	add start */
	if(is_unlocked()){
			NVT_LOG("unlock is true,phone is in cts test state!\n");
			is_cts_test = true;
		}
	else{
			NVT_LOG("phone is not in cts test state,normal boot!\n");
			is_cts_test = false;
	}
/* YunRui.Chen@RM.BSP.TP.FUNCTION, 2018/12/7, Add for cts gsi test  add end */	
