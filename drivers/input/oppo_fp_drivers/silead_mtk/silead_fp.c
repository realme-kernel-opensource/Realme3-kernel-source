...
#ifdef TURN_ON_CHARGE_FUC
extern unsigned int upmu_get_rgs_chrdet(void);
static int silead_get_usb_state(char *charge_state)
{
    int ret = 0;
    ret = upmu_get_rgs_chrdet();
    LOG_MSG_DEBUG(INFO_LOG,"%s get usb status %d\n", __func__, ret);
	if(1==ret){
    	*charge_state = 1;
	}else{
		*charge_state = 0;
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
 ...
 /* guard against device removal before, or while,
    ...
    case SIFP_IOC_DEINIT:
        ...
        break;
	case SIFP_IOC_CHARGE_STATE:
		#ifdef TURN_ON_CHARGE_FUC
        if (arg) {		
			char charge_state = 0;
			silead_get_usb_state(&charge_state);

	        if (copy_to_user((void __user *)arg, (void *)&charge_state, sizeof(charge_state))) {
	            retval = -EFAULT;
	        }
        }
		#endif
...
{
    int status = 0;
    printk("-------------%s line=%d: entry 1111111111111-------------\n",__func__,__LINE__);
 ...
 /* Claim our 256 reserved device numbers.  Then register a class
     ...
     */
     if (FP_SILEAD_6150 != get_fpsensor_type()){
		status = -EINVAL;
		printk("%s line=%d,found not SILEAD fp sensor status = %d\n", __func__, __LINE__, status);
		return status;
	 }
    status = register_chrdev(FP_DEV_MAJOR, "sil", &silfp_dev_fops);
    if (status < 0) {
		printk("-------------%s line=%d: Failed to register char device-------------\n",__func__,__LINE__);
        return status;
    }
    ...
    if (IS_ERR(silfp_class)) {
		...
		printk("-------------%s line=%d: Failed to class_create-------------\n",__func__,__LINE__);
	...
	printk("-------------%s line=%d: silfp_dev_init succeed-------------\n",__func__,__LINE__);
    return status;
}
