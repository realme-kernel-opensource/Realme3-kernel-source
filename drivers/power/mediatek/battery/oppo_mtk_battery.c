...
/*Jun.Wei@RM.BSP.CHG.Basic, 2018/10/23, no pull down resistor */
...
static int meter_fg_30_get_average_current(void)
{
	int ret = 0;
    bool volid;
	/*Jun.Wei@RM.BSP.CHG.Basic, 2018/10/22, get real time current */
...
static void meter_fg_30_set_battery_full(bool full)
{
	if(last_full != full) {
		if (full) {
            /*Jun.Wei@RM.BSP.CHG.Basic, 2018/11/28, charging full notify gauge daemon direct*/
			notify_fg_chr_full();
			last_full = full;
...
#define BAT_ID_2 (2) //AUXIN2
#define BAT_ID_3 (3) //AUXIN3
static int battery_type_check(void)
{
	...
	int ret_value = 0;
	int times = 5;
	int channel = BAT_ID_2;
	int battery_type = BAT_TYPE__UNKNOWN;

    /*Jun.Wei@RM.BSP.CHG.Basic, 2019/01/25, diff BAT ID ADC chanel in 18611*/
    if (is_project(OPPO_18611) || is_project(OPPO_17065)) {
        channel = BAT_ID_3;
    }
	...
	if (IMM_IsAdcInitReady() == 0) {
	 ...
	 i = times;
	 while (i--) {
        printk("IMM_GetOneChannelValue start channel=%d\n", channel);
