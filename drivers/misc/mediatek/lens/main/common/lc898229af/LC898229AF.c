...
static int setPosition(unsigned short UsPosition)
{
	...
	*/
	...
	//LOG_INF("DAC(%04d) -> 0x%04x\n", UsPosition, TarPos);
	LOG_INF("UcPosH = 0x%02x, UcPosL = 0x%02x \n", UcPosH, UcPosL);
...
static inline int initdrv(void)
{
	...
	s4AF_ReadReg(0xF0, &Temp);
	if (Temp == 0xA5) {
		LOG_INF("read the ID 0x%x\n", Temp);
		while(1) {
			mdelay(1);
			/*check LSI wake up or not*/
			ret = s4AF_ReadReg(0xB3, &Temp);
			//LOG_INF("read the 0xB3 reg = 0x%x\n", Temp);
			Temp &= 0xE0;
			//LOG_INF("read the 0xB3 & 0xE0 reg = 0x%x\n", Temp);
			if (ret == 0 && Temp == 0) {
				i4RetValue = 1;
				break;
			}
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		if (initdrv() == 1) {
			...
			//if (g_MotorDirection && g_MotorResolution) {
				int Step = 0;
				//LOG_INF("will setPosition a_u4Position =%llu\n", a_u4Position);
				LOG_INF("initdrv  done\n");

				while (Step < a_u4Position) {
					Step += (a_u4Position / VCM_STEP);
					LOG_INF(" setPosition Step =%d\n", Step);
					setPosition((unsigned short)Step);
					mdelay(10);
				}
			//}
...
/* ////////////////////////////////////////////////////////////// */
long LC898229AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int LC898229AF_PowerDown(void)
{
	...
	if (*g_pAF_Opened == 0) {
		...
		if (g_TimeOutChk == 0) {
			...
			LOG_INF("LC898229AF Power Down = %d\n", CurTime);
		} else {
...
int LC898229AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
...
int LC898229AFA_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	g_pstAF_I2Cclient = pstAF_I2Cclient;
	g_pAF_SpinLock = pAF_SpinLock;
	g_pAF_Opened = pAF_Opened;
...
int LC898229AFB_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	g_pstAF_I2Cclient = pstAF_I2Cclient;
	g_pAF_SpinLock = pAF_SpinLock;
	g_pAF_Opened = pAF_Opened;
...
int LC898229AFC_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	g_pstAF_I2Cclient = pstAF_I2Cclient;
	g_pAF_SpinLock = pAF_SpinLock;
	g_pAF_Opened = pAF_Opened;
