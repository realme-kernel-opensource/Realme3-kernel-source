...
/* Qiao.Hu@BSP.BaseDrv.CHG.Basic, 2018/04/16, modify for otg */
	...
	if (mtk_idpin_cur_stat == IDPIN_OUT) {
       ...
       mtk_xhci_mtk_printk(K_ALET, "000 current mode is %s, ret(%d)\n",
       ...
       mtk_xhci_mtk_printk(K_ALET, "001 current mode is %s, ret(%d)\n",
                (mtk_idpin_cur_stat == IDPIN_IN_HOST) ? "host" :
                (mtk_idpin_cur_stat == IDPIN_IN_DEVICE) ? "id_device" : "device",
                ret);
        ...
        /*Jun.Wei@RM.BSP.CHG.Basic, 2018/12/17, add for eye parameter*/
		set_usb_phy_mode(2); // lgxotg
...
static int otg_iddig_probe(struct platform_device *pdev)
{
	...
	/*Jun.Wei@RM.BSP.CHG.Basic, 2018/10/22, add for GPIO42 MT6370 OTG enable */
	static struct pinctrl_state *pinctrl_chg_otg;
#ifndef VENDOR_EDIT
...
//Fuchun.Liao@BSP.CHG.Basic 2017/11/22 modify for otg
	...
	if (IS_ERR(pinctrl)) {
		...
		#ifdef ODM_HQ_EDIT
		/* hanwenchao@ODM.Bsp.OTG  2018/10/09 add for otg bring up */
		//return -1; //From MTK
		#else
		return -1;
		#endif //ODM_HQ_EDIT
	...
	/*Jun.Wei@RM.BSP.CHG.Basic, 2018/10/22, add for GPIO42 MT6370 OTG enable */
	pinctrl_chg_otg = pinctrl_lookup_state(pinctrl, "gpio42_output_high");
	if (IS_ERR(pinctrl_chg_otg))
		   dev_err(&musb_pltfm_dev->dev, "Cannot find usb pinctrl gpio42_output_high\n");
	else
		   pinctrl_select_state(pinctrl, pinctrl_chg_otg);
	// add end
