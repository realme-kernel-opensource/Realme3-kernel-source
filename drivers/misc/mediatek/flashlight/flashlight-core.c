...
#endif
#ifdef VENDOR_EDIT
/*Zezhi.Wang@Camera.Driver 20181127 add for multi project using one build*/
                                                                     ...
                                                                     loff_t *data)
{
	...
	if(regBuf[0] == '0') {
		...
		fl_dev_arg.channel = fdev->dev_id.channel;
		/*Wangzezhi_hq@Camera.Driver 20181024 modify for improve brightness of flashlight */
		fl_dev_arg.arg = 6;
		if (fdev->ops) {
			fdev->ops->flashlight_ioctl(FLASH_IOC_SET_DUTY, (unsigned long)&fl_dev_arg);
		        /*Wangzezhi_hq@Camera.Driver 20181024 modify for improve brightness of flashlight */
...
void FlashRegulatorCtrl(int Stage)
{
	/*Zezhi.Wang@Camera.Driver 20181127 add for we don.t need other project's customize*/
...
/* caibowen@HQ 20190105 , add for flashing too short */
