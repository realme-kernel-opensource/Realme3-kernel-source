...
#define PFX "S5K3l6_sub_qt_camera_sensor"
...
#include "s5k3l6_sub_qt_mipiraw_Sensor.h"

#include <linux/hardware_info.h> //For hardwareinfo

#define MULTI_WRITE 1

#define EEPROM_GT24C64_ID 0xA4
#define S5K3L6_QT_EEPROM_CALI
#if MULTI_WRITE
...
//modify dualcam FAE IDLHLYE-99 by wei.miao@reallytek.com 180619 start
...
/* Camera Hardwareinfo */
extern struct global_otp_struct hw_info_sub_otp;

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static struct imgsensor_info_struct imgsensor_info = {
        .sensor_id = S5K3L6_SUB_QT_SENSOR_ID,
        ...
        //wangzezhi@ODM_HQ.camera.driver reduce isp driver current for RF conference
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
 ...
 {4208, 3120, 824, 600, 2560, 1920,  640,  480,   0,   0,  640,  480,   0, 0,  640,  480}, /* HS_hight speed video */
 {4208, 3120, 544, 480, 3120, 2160, 3840, 2160,   0,   0, 1920, 1080,   0, 0, 1920, 1080}, /* slim video */
};
/*
...
void s5k3l6_read_OTP(struct s5k3l6_txd_otp_data_str *otp)
{
        unsigned char val = 0;

        LOG_INF("s5k3l6_read_OTP start.\n");

        val=read_cmos_sensor_byte(0x0000);//flag of info and awb1
        LOG_INF("flag of info and awb 1:%d.\n", val);

        if(val == 0x01)
        {
                otp->info_flag = 0x01;
                otp->supplier_code= read_cmos_sensor_byte(0x0001);
                otp->module_code= read_cmos_sensor_byte(0x0002);
                otp->module_version= read_cmos_sensor_byte(0x0003);
                otp->software_version= read_cmos_sensor_byte(0x0004);
                otp->year= read_cmos_sensor_byte(0x0005);
                otp->month= read_cmos_sensor_byte(0x0006);
                otp->day= read_cmos_sensor_byte(0x0007);
                otp->awb_flag= read_cmos_sensor_byte(0x0010);
         if(otp->awb_flag)
         {
                        otp->R_over_G_unit= (read_cmos_sensor_byte(0x0011)<<8)+read_cmos_sensor_byte(0x0012);
                        otp->B_over_G_unit= (read_cmos_sensor_byte(0x0013)<<8)+read_cmos_sensor_byte(0x0014);
                        otp->Gb_over_Gr_unit= (read_cmos_sensor_byte(0x0015)<<8)+read_cmos_sensor_byte(0x0016);
                otp->R_over_G_golden=(read_cmos_sensor_byte(0x0017)<<8)+read_cmos_sensor_byte(0x0018);
                otp->R_over_G_golden=(read_cmos_sensor_byte(0x0019)<<8)+read_cmos_sensor_byte(0x001A);
                otp->R_over_G_golden=(read_cmos_sensor_byte(0x001B)<<8)+read_cmos_sensor_byte(0x001C);
                otp->awb_checksum=read_cmos_sensor_byte(0x001D);
         }
        ...
        LOG_INF("s5k3l6_read_OTP complete.\n");
}
void s5k3l6_apply_OTP(struct s5k3l6_otp_struct *s5k3l6_OTP)
{
        int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;

         LOG_INF("s5k3l6_apply_OTP start.\n");
        ...
        if(((s5k3l6_OTP->flag)&0x03) != 0x01)
        {
                LOG_INF("s5k3l6_apply_OTP error.\n");
                return;
        }

        R_gain = (RGr_ratio_Typical*1000) / s5k3l6_OTP->RGr_ratio;
        B_gain = (BGr_ratio_Typical*1000) / s5k3l6_OTP->BGr_ratio;
        Gb_gain = (GbGr_ratio_Typical*1000) / s5k3l6_OTP->GbGr_ratio;
        Gr_gain = 1000;
        Base_gain = R_gain;
        ...
        LOG_INF("s5k3l6_apply_OTP complete.\n");
}
        ...
        while (imgsensor_info.i2c_addr_table[i] != 0xff) {
                ...
                do {
                        *sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001)) + 2;
                        pr_err("s5k3l6_sub_qt_sensor_id:0x%x",*sensor_id);
                        if (*sensor_id == imgsensor_info.sensor_id) {
                                hw_info_sub_otp.sensor_name = SENSOR_DRVNAME_S5K3L6_SUB_QT_MIPI_RAW;
                                //modify by zaki for third vendor module++++++++++
#ifdef S5K3L6_QT_EEPROM_CALI
                                vendorId = read_eeprom(0x0000);
                                pr_err("i2c write id: 0x%x, sensor id: 0x%x vendorID=0x%x\n",
                                        imgsensor.i2c_write_id, *sensor_id,vendorId);
                                if (vendorId != 0x09)
                                {

//                                        hw_info_main_otp.sensor_name = SENSOR_DRVNAME_S5K3L6_SUB_QT_MIPI_RAW;
...
{
        /*Xiaoyang.Huang@RM.Camera add for resolve isp deque fail 20190112*/
...
UINT32 S5K3L6_SUB_QT_MIPI_RAW_SensorInit(
