...
enum IMGSENSOR_RETURN imgsensor_hw_init(struct IMGSENSOR_HW *phw)
{
	...
	int i, j;
	#ifdef VENDOR_EDIT
	/*Zezhi.Wang@Camera.Driver add for 18601  for camera i2c pullup disable after DVT board 20181128*/
	struct pinctrl *pinctrl;
	struct pinctrl_state *pins_cfg0=0;
	struct pinctrl_state *pins_cfg1=0;
	struct pinctrl_state *pins_cfg2=0;
	struct pinctrl_state *pins_cfg3=0;
	#endif

	/*Zezhi.Wang@Camera.Driver add for 18601  for camera i2c pullup disable after DVT board 20181128*/
	#ifdef VENDOR_EDIT
	pr_err("[i2cpullup] get_PCB_Version()=%d\n",get_PCB_Version());
	if ( get_PCB_Version() > preDVT )
	{
		pinctrl = devm_pinctrl_get(&phw->common.pplatform_device->dev);
		if(IS_ERR(pinctrl)){
			pr_err("[i2cpullup] Cannot find out pinctrl,pinctrl is null\n");
			return 0;
		}
		pins_cfg0 = pinctrl_lookup_state(pinctrl, "cami2c_pullup0");
		if(IS_ERR(pins_cfg0)){
			pr_err("[i2cpullup] Cannot find out pins_cfg0,pins_cfg0 is null\n");
		}
		pins_cfg1 = pinctrl_lookup_state(pinctrl, "cami2c_pullup1");
		if(IS_ERR(pins_cfg1)){
			pr_err("[i2cpullup] Cannot find out pins_cfg1,pins_cfg1 is null\n");
		}
		pins_cfg2 = pinctrl_lookup_state(pinctrl, "cami2c_pullup2");
		if(IS_ERR(pins_cfg2)){
			pr_err("[i2cpullup] Cannot find out pins_cfg2,pins_cfg2 is null\n");
		}
		pins_cfg3 = pinctrl_lookup_state(pinctrl, "cami2c_pullup3");
		if(IS_ERR(pins_cfg3)){
			pr_err("[i2cpullup] Cannot find out pins_cfg3,pins_cfg3 is null\n");
		}
		if(IS_ERR(pins_cfg0)||IS_ERR(pins_cfg1)||IS_ERR(pins_cfg2)||IS_ERR(pins_cfg3))
		{
			pr_err("[i2cpullup] pinctrl is_err!Do nothing\n");
		}else
		{
			pinctrl_select_state(pinctrl,pins_cfg0);
			pinctrl_select_state(pinctrl,pins_cfg1);
			pinctrl_select_state(pinctrl,pins_cfg2);
			pinctrl_select_state(pinctrl,pins_cfg3);
			pr_err("[i2cpullup] pinctrl i2c pullup finished!\n");
		}
	}
	#endif
...
struct IMGSENSOR_HW_CFG *imgsensor_hw_get_cfg(enum IMGSENSOR_SENSOR_IDX sensor_idx)
{
	struct IMGSENSOR_HW_CFG *pcust_cfg;
		...
		} else if (is_project(OPPO_18311) || is_project(OPPO_18011) || is_project(OPPO_18611)) {
			PK_INFO("This project is 18311/18011 and Operator_Version = %d\n", get_Operator_Version());
