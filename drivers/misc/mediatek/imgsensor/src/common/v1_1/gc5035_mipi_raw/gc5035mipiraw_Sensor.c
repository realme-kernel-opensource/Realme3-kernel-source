...
static void write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == GC5035_SENSOR_ID)
				&& (data_base == 0x1600)
				&& (data_length == 1561)) {
			pr_debug("gc5035 Write start: %x %x %x %x\n", pData[0], pData[1], pData[2], pData[3]);
			idx = data_length/8;
			idy = data_length%8;
			...
			table_write_eeprom_8Bytes((data_base+8*i), &pData[8*i], idy);
			msleep(6);
			pr_debug("gc5035 read 0x1600:0x%x\n", read_cmos_eeprom_8(0x1600));
			msleep(6);
			pr_debug("gc5035 read 0x1601:0x%x\n", read_cmos_eeprom_8(0x1601));
			msleep(6);
			pr_debug("gc5035 read 0x1C10:0x%x\n", read_cmos_eeprom_8(0x1C10));
			msleep(6);
			pr_debug("gc5035 read 0x1C11:0x%x\n", read_cmos_eeprom_8(0x1C11));
			msleep(6);
			pr_debug("gc5035 Write end\n");
		}
	} else {
		pr_err("gc5035 pStereodata is null\n");
	}
}
#endif
