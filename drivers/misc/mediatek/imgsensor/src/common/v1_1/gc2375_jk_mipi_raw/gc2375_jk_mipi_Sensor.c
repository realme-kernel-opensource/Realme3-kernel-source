...
#include "gc2375_jk_mipi_Sensor.h"

#include <linux/hardware_info.h> //For hardwareinfo

/****************************Modify Following Strings for Debug****************************/
#define PFX "gc2375_jk_camera_sensor"
...
static kal_bool GC2375HDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
        .sensor_id = GC2375_JK_SENSOR_ID,                //record sensor id defined in Kd_imgsensor.h
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] =
{
        ...
        { 1600, 1200,         0,        0, 1600, 1200, 1600,  1200, 0000, 0000, 1600,  1200,          0,        0, 1600,  1200}  // 24fps dual camera
};
...
#if 0//def GC2375H_MIPI_USE_OTP        //Do not use these code because we use EEPROM to store the data
...
#ifdef GC2375H_MIPI_USE_OTP
static struct gc2375h_otp_data_str pOTP_Data = {0};

static kal_uint32 read_eeprom_info_data(struct gc2375h_otp_data_str *pData, kal_uint16 addr)
{
        ...
        pData->info_checksum_calc = checksum % 255 +1;
        pData->info_checksum = read_eeprom(0x0F) & 0xFF;//0x0F: Info data checksum
        LOG_DBG("info_checksum_calc = 0x%x, info_checksum = 0x%x\n",
                        pData->info_checksum_calc, pData->info_checksum);

        return ret;
}

static kal_uint32 read_eeprom_awb_data(struct gc2375h_otp_data_str *pData, kal_uint16 addr)
{
        kal_uint32 i = 0, ret = 0, checksum = 0;
        u8 *p = (u8 *)(&(pData->R_over_G_unit_h));
        ...
        pData->awb_checksum_calc = checksum % 255 + 1;
        pData->awb_checksum = read_eeprom(0x1F) & 0xFF;//0x1F: AWB data checksum
        LOG_DBG("awb_checksum_calc = 0x%x, awb_checksum = 0x%x\n",
                        pData->awb_checksum_calc, pData->awb_checksum);

        return ret;
}

static kal_uint32 apply_eeprom_awb(struct gc2375h_otp_data_str *pData)
{
        //kal_uint32 Gb_test1, Gr_test1;
        kal_uint32 R_gain,B_gain,G_gain,Base_gain; /*,Gb_test*/
        //kal_uint32 tempR, tempB, tempG, tempGr, tempGb;
        kal_uint16 RoverG_dec,BoverG_dec;
        kal_uint16 RoverG_dec_base,BoverG_dec_base;

        RoverG_dec  = (pData->R_over_G_unit_h << 8) | (pData->R_over_G_unit_l & 0x00FF);
        BoverG_dec  = (pData->B_over_G_unit_h << 8) | (pData->B_over_G_unit_l & 0x00FF);
        //GboverGr_dec = (pData->Gb_over_Gr_unit_h << 8) | (pData->Gb_over_Gr_unit_l & 0x00FF);
        LOG_DBG("RoverG_dec = %d, BoverG_dec = %d",        RoverG_dec, BoverG_dec);

        RoverG_dec_base = (pData->R_over_G_golden_h << 8) | (pData->R_over_G_golden_l & 0x00FF);
        BoverG_dec_base  = (pData->B_over_G_golden_h << 8) | (pData->B_over_G_golden_l & 0x00FF);
        //GboverGr_dec_base = (pData->Gb_over_Gr_golden_h << 8) | (pData->Gb_over_Gr_golden_l & 0x00FF);
        LOG_DBG("RoverG_dec_base = %d, BoverG_dec_base = %d", RoverG_dec_base, BoverG_dec_base);

        if (RoverG_dec_base == 0 || BoverG_dec_base == 0)// || GboverGr_dec_base == 0)
                return false;
        if (RoverG_dec == 0 || BoverG_dec == 0)// || GboverGr_dec == 0)
                return false;

        R_gain=(RoverG_dec_base*1024)/RoverG_dec;
        B_gain=(BoverG_dec_base*1024)/BoverG_dec;
        G_gain=1024;
        ...
        /*TODO*/
        write_cmos_sensor(0xfe,0x00);
        write_cmos_sensor(0xb8,G_gain>>3);
        write_cmos_sensor(0xb9,R_gain>>3);
        write_cmos_sensor(0xba,B_gain>>3);
        write_cmos_sensor(0xbb,G_gain>>3);
        write_cmos_sensor(0xbe,((G_gain&0x07) << 4) + (R_gain&0x07));
        write_cmos_sensor(0xbf,((B_gain&0x07) << 4) + (G_gain&0x07));
        write_cmos_sensor(0xfe,0x00);

        return 1;
}

static void get_eeprom_data(struct gc2375h_otp_data_str *pData)
{
        pData->info_flag = read_eeprom(0x00);//Info data flag[0x01:Valid]
        if (pData->info_flag == 0x01) {
                read_eeprom_info_data(pData, 0x01);//Info data begin from 0x01
        }
        pData->awb_flag = read_eeprom(0x10);//AWB data flag[0x01:Valid]
        if (pData->awb_flag == 0x01) {
                read_eeprom_awb_data(pData, 0x11);//AWB data begin from 0x11
        }
}
#endif
        ...
        while (imgsensor_info.i2c_addr_table[i] != 0xff) {
                ...
                do {
                        *sensor_id = return_sensor_id();
                        pr_err("gc2375H_sensor_id:0x%x",*sensor_id);
                        if (*sensor_id == imgsensor_info.sensor_id) {
                                hw_info_main2_otp.sensor_name = SENSOR_DRVNAME_GC2375_JK_MIPI_RAW;
                                LOG_DBG("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
#ifdef GC2375H_MIPI_USE_OTP
                                /* Read EEPROM data if sensor id is right */
                                get_eeprom_data(&pOTP_Data);

                                hw_info_main2_otp.sensor_name = SENSOR_DRVNAME_GC2375H_MIPI_RAW;
                                hw_info_main2_otp.otp_valid = pOTP_Data.info_flag ? 1 : 0;
                                hw_info_main2_otp.vendor_id = pOTP_Data.supplier_code;
                                hw_info_main2_otp.module_code = pOTP_Data.module_code;
                                hw_info_main2_otp.module_ver = pOTP_Data.module_version;
                                hw_info_main2_otp.sw_ver = pOTP_Data.software_version;
                                hw_info_main2_otp.year = pOTP_Data.year;
                                hw_info_main2_otp.month = pOTP_Data.month;
                                hw_info_main2_otp.day = pOTP_Data.day;
                                hw_info_main2_otp.vcm_vendorid = 0;
                                hw_info_main2_otp.vcm_moduleid = 0;
#endif
...
#ifdef GC2375H_MIPI_USE_OTP
        /* Retry read awb if get invalid awb data in sensor search */
        if ((pOTP_Data.awb_flag != 0x01) ||
                        (pOTP_Data.awb_checksum_calc != pOTP_Data.awb_checksum)) {
                pOTP_Data.awb_flag = read_eeprom(0x10);
                if (pOTP_Data.awb_flag == 0x01)
                        read_eeprom_awb_data(&pOTP_Data, 0x11);
        }
        //Apply AWB
        if ((pOTP_Data.awb_flag == 0x01) &&
                        (pOTP_Data.awb_checksum_calc == pOTP_Data.awb_checksum))
                apply_eeprom_awb(&pOTP_Data);
#endif
...
UINT32 GC2375_JK_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
