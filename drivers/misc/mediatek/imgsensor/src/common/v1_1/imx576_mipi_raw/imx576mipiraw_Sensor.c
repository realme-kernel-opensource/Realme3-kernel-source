...
/*Feng.Hu@Camera.Driver 20171230 add for imx576 0.9cut and 0.91cut*/
...
#if USE_REMOSAIC
{ 5760,  4312,  0,  0,  5760,  4312,  5760,  4312,  0000,  0000,  5760,  4312,  0,  0,  5760,  4312}, /* remosic*/
#else
{ 2880,  2156,  0,  0,  2880,  2156,  2880,  2156,  0000,  0000,  2880,  2156,  0,  0,  2880,  2156}, /* capture*/
#endif
...
static kal_uint32 return_lot_id_from_otp(void)
{
	...
	sensor_id = (read_cmos_sensor_8(0x0A22) << 4) | (read_cmos_sensor_8(0x0A23) >> 4);
			...
			*sensor_id = return_lot_id_from_otp();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_IMX576, module_id);
...
static kal_uint32 open(void)
{
	...
	if (chip_id <= 0x10) {
		...
		pr_err("ophf the val of 0x0018 is %d\n", read_cmos_sensor_8(0x0018));
	}
	#endif
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	#ifndef VENDOR_EDIT
	/*Caohua.Lin@Camera.Driver 20180224 to disable hdr with 0.90 sensor*/
							 ...
							 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		...
		if (imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE) {
			...
			imx576_awb_gain((SET_SENSOR_AWB_GAIN *) feature_para);
		}
		#endif
		break;
