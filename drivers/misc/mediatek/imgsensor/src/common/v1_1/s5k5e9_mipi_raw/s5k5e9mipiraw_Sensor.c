...
bool start_read_otp(int Page,kal_uint8* data)
{
   ...
   {
       LOG_INF("start_read_otp :Page %d read otp fail\n",Page);
       return 0;
   }
...
void module_info_otp_read(void)
{
	...
	LOG_INF("module_info_otp_read");

	ret = start_read_otp(19,otp_data);
	if (ret != 1)
		ret = start_read_otp(17,otp_data);
	if (ret != 1)
		LOG_INF("module_info_otp_read fail");
...
//kin0603
UINT32 S5K5E9_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
