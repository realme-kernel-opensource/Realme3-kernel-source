...
#define DEVICE_VERSION_IMX519     "imx519"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX519_SENSOR_ID,	/* record sensor id defined in Kd_imgsensor.h */

	.checksum_value = 0x1c0140cc,	/* checksum value for Camera Auto Test */

	//preview
	.pre = {
		.pclk = 500000000,	/* record different mode's pclk */
		.linelength = 5920,	/* record different mode's linelength */
		.framelength = 2815,	/* record different mode's framelength */
	...
	.i2c_addr_table = {0x34, 0x35, 0xff},	/* record sensor support all write id addr,
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	{4656, 3496, 000, 000, 4656, 3496, 2328, 1728, 0000, 0000, 2328, 1728, 0, 0, 2328, 1728},	/* Preview */
	{4656, 3496, 000, 000, 4656, 3496, 4656, 3496, 0000, 0000, 4656, 3496, 0, 0, 4656, 3496},	/* capture */
	{4656, 3496, 000, 440, 4656, 2618, 4656, 2618, 0000, 0000, 4656, 2618, 0, 0, 4656, 2618},	/* video */
	{4656, 3496, 408, 668, 3840, 2160, 1920, 1080, 0000, 0000, 1920, 1080, 0, 0, 1920, 1080},	/* hight speed video */
	{4656, 3496, 000, 000, 4656, 3496, 4656, 3496, 0000, 0000, 4656, 3496, 0, 0, 4656, 3496},	/*  custom1 */
	{4656, 3496, 696, 504, 3264, 2488, 3264, 2488, 0000, 0000, 3264, 2488, 0, 0, 3264, 2488},	  /* custom2 */
};
...
static SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[4] = {
	...
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x14E0, 0x0FB0, 0x00, 0x35, 0x0280, 0x0001,
	 0x00, 0x36, 0x1230, 0x0001, 0x03, 0x00, 0x0000, 0x0000},
	 /* HighSpeed mode setting */
	 {0x03, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x14E0, 0x0FB0, 0x00, 0x35, 0x0280, 0x0001,
	 0x00, 0x36, 0x0960, 0x0001, 0x03, 0x00, 0x0000, 0x0000},
};
...
static void imx519_apply_LRC(void)
{
	...
	iBurstWriteReg_multi(puSendCmd + 72, 72, imgsensor.i2c_write_id, 72,
			 400);

	LOG_INF("readback LRC, L1(%d) L70(%d) R1(%d) R70(%d)\n",read_cmos_sensor_8(0x7520),
		read_cmos_sensor_8(0x7565),read_cmos_sensor_8(0x7568),read_cmos_sensor_8(0x75AD));
...
#endif
static kal_uint16 imx519_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static void preview_setting(void)
{
	...
	//PDAF Area Config Begin
    write_cmos_sensor_8(0x38A3,0x02);//0:Fixed (16x12), 1:Fixed (8x6), 2:Flexible
    ...
    write_cmos_sensor_8(0x38AC,0x01);// enable and disable Flexible window,total 8
...
{
	LOG_INF("capture_setting() 60 fps E! currefps:%d\n", currefps);
	#if USE_BURST_MODE
	...
	#endif
	LOG_INF("capture_setting(PD 012515) 60 fpsX\n");
}
#else
//full size 30fps
static void capture_setting(kal_uint16 currefps)
{
	LOG_INF("capture_setting(PD 012515) 30 fps E! currefps:%d\n", currefps);
	/*************MIPI output setting************/
	#if USE_BURST_MODE
	...
	write_cmos_sensor_8(0x30AC,0x00);
	#endif
	LOG_INF("capture_setting(PD 012515) 30 fpsX\n");
}
#endif
...
static void normal_video_setting(kal_uint16 currefps)
{
    ...
    write_cmos_sensor_8(0x38B7,0x1E);//798
    write_cmos_sensor_8(0x38B8,0x0C);
    write_cmos_sensor_8(0x38B9,0x75);//3189
    write_cmos_sensor_8(0x38BA,0x07);
    write_cmos_sensor_8(0x38BB,0x1B);//1819
...
{
    LOG_INF("hs_video_setting()+++ E! currefps 120\n");
    /*************MIPI output setting************/
    write_cmos_sensor_8(0x0111, 0x03);
    write_cmos_sensor_8(0x0112, 0x0A);
    write_cmos_sensor_8(0x0113, 0x0A);
    ...
    write_cmos_sensor_8(0x38B4,0x02);
    write_cmos_sensor_8(0x38B5,0x5e);//
    write_cmos_sensor_8(0x38B6,0x01);
    write_cmos_sensor_8(0x38B7,0x4A);//
    write_cmos_sensor_8(0x38B8,0x05);
    write_cmos_sensor_8(0x38B9,0x22);//
    write_cmos_sensor_8(0x38BA,0x02);
    write_cmos_sensor_8(0x38BB,0xED);//
...
{
	LOG_INF("custom1_setting() 30 fps E! currefps\n");
	/*************MIPI output setting************/
	#if USE_BURST_MODE
...
{
	LOG_INF("custom2_setting() 30 fps E! currefps \n");
	/*************MIPI output setting************/
	#if USE_BURST_MODE
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017));
			LOG_INF("read_0x0000=0x%x 11111, 0x0001=0x%x,0x0000_0001=0x%x\n",
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_b", DEVICE_VERSION_IMX519, module_id);
			...
			LOG_INF("Read sensor id fail 22222, id: 0x%x\n", imgsensor.i2c_write_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
//			read_3P8_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
		...
		case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
			...
			imx519_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32), feature_data_16);
			break;
		case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
			LOG_INF("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d", (*feature_para_len));
			imx519_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32), feature_data_16);
			break;
