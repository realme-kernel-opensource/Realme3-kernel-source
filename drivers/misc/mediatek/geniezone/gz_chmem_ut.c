...
struct _ssmr_chmem_regions _ssmr_CM_ary[] = {
	...
	{SSMR_FEAT_PROT_SHAREDMEM, MTEE_MCHUNKS_PROT},             /*MTEE/TEE Protect-shared*/
	{SSMR_FEAT_TA_ELF, MTEE_MCHUNKS_HAPP},				       /*HA ELF*/
	{SSMR_FEAT_TA_STACK_HEAP, MTEE_MCHUNKS_HAPP_EXTRA},        /*HA Stack/Heap*/
	{SSMR_FEAT_SDSP_FIRMWARE, MTEE_MCHUNKS_SDSP},              /*SDSP Firmware*/
	{SSMR_FEAT_SDSP_TEE_SHAREDMEM, MTEE_MCHUNKS_SDSP_SHARED},  /*SDSP/TEE Shared*/
#else
	{SSMR_FEAT_PROT_SHAREDMEM, 0}                             /*MTEE/TEE Protect-shared*/
#endif
};
	...
	, KREE_HANDLE append_hd)
{
	...
	if (test_type == (uint32_t) ION_chmem)
		...
		ret = KREE_ReleaseSecureMultichunkmem_basic(append_sn, append_hd);
	...
	int alloc_size, int alignment, int flags)
{
	...
	if (flags == 0)
		...
		if (test_type == (uint32_t) ION_chmem)
			...
			ret = KREE_ZallocSecureMultichunkmem(alloc_sn, append_hd,
				alloc_hd, alignment, alloc_size);
	...
	/*
	 * KREE_DEBUG("[OK]Alloc chmem [handle=0x%x, mem_handle=0x%x (align=0x%x, size=0x%x)]\n",
		...
		KREE_HANDLE *alloc_hd)
{
	...
	TEST_BEGIN("====> call _alloc_chmem_and_check");
	/*RESET_UNITTESTS;*/
	...
	KREE_SESSION alloc_sn, KREE_HANDLE alloc_hd)
{
	...
	TEST_BEGIN("====> call _free_chmem_and_check");
	/*RESET_UNITTESTS;*/
	...
	KREE_HANDLE alloc_hd)
{
	...
	TEST_BEGIN("====> call _ref_chmem_and_check");
	/*RESET_UNITTESTS;*/
		...
		KREE_HANDLE alloc_hd, uint32_t cmd)
{
	...
	TEST_BEGIN("====> call _query_chmem_and_check");
	/*RESET_UNITTESTS;*/
		...
		int *_alloc_num, KREE_HANDLE **alloc_hd)
{
	...
	TEST_BEGIN("====> start to test:_test_saturation_sequential_body");
	/*RESET_UNITTESTS;*/
	...
	*/
	...
	for (n = _test_CM_idx, m = 0; n < end_idx; n++, m++) {
		...
		for (i = 0; i < _alloc_num[m]; i++) {
			...
			ret = _alloc_chmem_and_check(test_type, alloc_sn, append_hd[m],
				alloc_size, alignment, flags, &alloc_hd[m][i]);
			if (ret != TZ_RESULT_SUCCESS) {
				KREE_ERR("alloc fail: i=%d\n", i);
				break;
			}
		...
		ret = _alloc_chmem(test_type, alloc_sn, &extra_alloc_hd, append_hd[m],
			alloc_size, alignment, flags);
		...
		for (i = 0; i < _alloc_num[m]; i++) {
			/* KREE_DEBUG("=====> Free %d/%d\n", i, (_alloc_num[m]-1)); */
			ret = _free_chmem_and_check(test_type, alloc_sn, alloc_hd[m][i]);
			if (ret != TZ_RESULT_SUCCESS) {
				KREE_ERR("free fail: i=%d\n", i);
				break;
			}
		...
		int alignment, int flags, int *_alloc_num, KREE_HANDLE **alloc_hd)
{
	...
	TEST_BEGIN("====> start to test:_test_saturation_parallel_body");
	/*RESET_UNITTESTS;*/
		...
		int *_alloc_num, KREE_HANDLE **alloc_hd)
{
	...
	TEST_BEGIN("====> start to test:_test_saturation_all_body");
	/*RESET_UNITTESTS;*/
	...
	*/
	...
	for (n = _test_CM_idx, m = 0; n < end_idx; n++, m++) {
		...
		for (i = 0; i < _alloc_num[m]; i++) {
			...
			/*ref*/
			ret = _ref_chmem_and_check(test_type, alloc_sn, alloc_hd[m][i]);
			if (ret != TZ_RESULT_SUCCESS) {
		...
		int *_alloc_num, KREE_HANDLE **alloc_hd)
{
	...
	TEST_BEGIN("====> start to test:_test_by_MTEE_TA_body");
	/*RESET_UNITTESTS;*/
	...
	*/
	...
	for (n = _test_CM_idx, m = 0; n < end_idx; n++, m++) {
		...
		for (i = 0; i < _alloc_num[m]; i++) {
			...
			param[0].value.a = append_hd[m]; /* secure memory handle */
			param[0].value.b = _test_CM_ary[n].size;	/*chunk memory size*/
			param[1].value.a = alloc_size;	/* alloc size */
			param[1].value.b = alignment; /*alignment*/

			paramTypes = TZ_ParamTypes2(TZPT_VALUE_INPUT, TZPT_VALUE_INOUT);

			/*TZCMD_TEST_QUERYCHM_TA_Normal   0x9990*/
			KREE_TeeServiceCall(echo_sn, 0x9990, paramTypes, param);

			CHECK_EQ(TZ_RESULT_SUCCESS, param[1].value.a, "Test by MTEE TA");
		...
		int alloc_size, int alignment, int flags, uint32_t test_fun)
{
	...
	TEST_BEGIN("====> start to test:_test_saturation_main");
	/*RESET_UNITTESTS;*/
	...
	*/
	...
	alloc_hd = kmalloc_array(_test_CM_num, sizeof(KREE_HANDLE *), GFP_KERNEL);
	if (!alloc_hd) {
	...
	for (n = _test_CM_idx, m = 0; n < end_idx; n++, m++) {
		...
		alloc_hd[m] = kmalloc_array(alloc_ary, sizeof(KREE_HANDLE), GFP_KERNEL);
		if (!alloc_hd[m]) {
			KREE_ERR("kmalloc alloc_hd[] Fail\n");
			goto out;
		}
	...
	if (test_fun == (uint32_t) _test_seq)
		ret = _test_saturation_sequential_body(test_type, alloc_sn, append_hd,
			alloc_size, alignment, flags, _alloc_num, alloc_hd);
	else if (test_fun == (uint32_t) _test_parall)
		ret = _test_saturation_parallel_body(test_type, alloc_sn, append_hd,
			alloc_size, alignment, flags, _alloc_num, alloc_hd);
			...
			KREE_HANDLE *append_hd)
{
	...
	for (i = _testParams_begIdx; i <= _testParams_endIdx; i++) {
		KREE_DEBUG("==>params[%d]: size=0x%x, align=0x%x, init0=0x%x\n",
			i, params[i].size, params[i].alignment,	params[i].flags);
		...
		_test_chmem_alloc_func test_func)
{
	...
	*/
	...
	append_hd = kmalloc_array(_test_CM_num, sizeof(KREE_HANDLE), GFP_KERNEL);
	if (!append_hd) {
	...
	for (i = _test_CM_idx, j = 0; i < end_idx; i++, j++) {
		...
		_test_CM_ary[i].pa, _test_CM_ary[i].size, _test_CM_ary[i].region_id);
