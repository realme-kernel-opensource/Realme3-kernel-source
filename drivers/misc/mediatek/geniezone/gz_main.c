									...
									int in_chm_size)
{
	...
	/*append chunk memory*/
	ret = KREE_AppendSecureMultichunkmem(appendchm_session, appendchm_handle, &shm_param);
	if (ret != TZ_RESULT_SUCCESS) {
								...
								KREE_SECUREMEM_HANDLE appendchm_handle)
{
	...
	ret = KREE_ReleaseSecureMultichunkmem(appendchm_session, appendchm_handle);
	if (ret != TZ_RESULT_SUCCESS) {
		KREE_ERR("KREE_ReleaseSecureMultichunkmem Error: ret=%d\n", ret);
		goto out1;
	}
	KREE_DEBUG("[OK][%d] release chm OK. appendchm_handle=0x%x\n",
					__LINE__, (uint32_t) appendchm_handle);
...
static long _sc_test_releaseChm(struct file *filep, unsigned long arg)
{
	...
	KREE_DEBUG("[%s]append_chm_handle=0x%x, append_chm_session =0x%x\n",
		__func__, (uint32_t) append_chm_handle, (uint32_t) append_chm_session);
...
static int _sc_test_allocChm_body(KREE_SESSION_HANDLE alloc_chm_session,
							...
							uint32_t alignment_size, uint32_t allocchm_size,
							int zalloc)
{
	...
	if ((alloc_chm_session == 0) || (appendchm_handle == 0)) {
		KREE_DEBUG("[%s]alloc_chm_session or appendchm_handle=0[Stop]\n", __func__);
		return TZ_RESULT_ERROR_BAD_PARAMETERS;
	}
	...
	*ION_Handle = 0;
	...
	/* allocate chm */
	if (zalloc)
		ret = KREE_ION_ZallocChunkmem(alloc_chm_session, appendchm_handle,
						&allocatechm_handle, alignment, alloc_size);
	else
		ret = KREE_ION_AllocChunkmem(alloc_chm_session, appendchm_handle,
						&allocatechm_handle, alignment, alloc_size);
	if ((ret != TZ_RESULT_SUCCESS) || (allocatechm_handle == 0))
		KREE_ERR("[%s] KREE_ION_AllocChunkmem() returns Fail. [Stop!]. ret=0x%x\n"
					, __func__, ret);
	else {
		KREE_DEBUG("[OK]Alloc chunk memory [chm_handle = 0x%x, mem_handle = 0x%x (alignment=%d, size=%d)]\n",
					appendchm_handle, allocatechm_handle, alignment, alloc_size);
...
static long _sc_test_allocChm(struct file *filep, unsigned long arg, int zalloc)
{
	...
	KREE_DEBUG("[%s]input allocchm_size = 0x%x,alloc_chm_session=0x%x\n",
			__func__, (uint32_t) allocchm_size, (uint32_t) alloc_chm_session);
	...
	ret = _sc_test_allocChm_body(alloc_chm_session, append_chm_handle,
						&ION_Handle, alignment_size, allocchm_size, zalloc);
	if (ret != TZ_RESULT_SUCCESS) {
...
static long _sc_test_refChm(struct file *filep, unsigned long arg)
{
	...
	KREE_DEBUG("[%s]ION_Handle = 0x%x,alloc_chm_session=0x%x\n",
			__func__, (uint32_t) ION_Handle, (uint32_t) alloc_chm_session);
