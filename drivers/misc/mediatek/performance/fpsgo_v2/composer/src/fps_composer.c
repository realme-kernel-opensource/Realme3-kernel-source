...
void fpsgo_ctrl2comp_enqueue_start(int pid,
	unsigned long long enqueue_start_time, unsigned long long identifier)
{
	...
	FPSGO_COM_TRACE("%s pid[%d] identifier %llu", __func__, pid, identifier);
	...
	if (!f_render->api && identifier) {
		...
		ret = fpsgo_get_BQid_pair(pid, f_render->tgid, identifier, &buffer_id, &queue_SF);
		if (!ret || !buffer_id) {
	...
	case VSYNC_ALIGNED_TYPE:
		...
		FPSGO_COM_TRACE("pid[%d] type[%d] enqueue_s:%llu frame_time:%llu buffer_id:%llu",
			pid, f_render->frame_type, enqueue_start_time, f_render->self_time, f_render->buffer_id);
...
void fpsgo_ctrl2comp_enqueue_end(int pid,
	unsigned long long enqueue_end_time, unsigned long long identifier)
{
	struct render_info *f_render;
	int xgf_ret = 0;
	int check_render;
	...
	case VSYNC_ALIGNED_TYPE:
		...
		fpsgo_comp2fstb_enq_end(f_render->pid, f_render->enqueue_length);
		break;
	case NON_VSYNC_ALIGNED_TYPE:
		f_render->t_enqueue_end = enqueue_end_time;
		f_render->enqueue_length =
...
void fpsgo_ctrl2comp_dequeue_start(int pid,
	unsigned long long dequeue_start_time, unsigned long long identifier)
{
	struct render_info *f_render;
	int xgf_ret = 0;
	int check_render;
...
void fpsgo_ctrl2comp_dequeue_end(int pid,
	unsigned long long dequeue_end_time, unsigned long long identifier)
{
	struct render_info *f_render;
	int xgf_ret = 0;
	int check_render;
	...
	int queue_SF, unsigned long long identifier, int create)
{
	...
	FPSGO_LOGI("bqid %d: buffer_id %llu, identifier %llu, queue_SF %d, create %d\n",
		pid, buffer_id, identifier, queue_SF, create);

	if (create) {
		pair = fpsgo_find_BQ_id(pid, 0, identifier, ACTION_FIND_ADD, 0ULL);
		...
		fpsgo_find_BQ_id(pid, 0, identifier, ACTION_FIND_DEL, buffer_id);
