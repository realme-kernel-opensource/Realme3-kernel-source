...
static void ext_lcd_bias_write_byte(unsigned char addr, unsigned char value)
{
    ...
    if (ret < 0)
        ...
        LCD_BIAS_PRINT("[LCD][BIAS] PXL i2c write data success!!\n");
}

static void ext_lcd_bias_set_vspn_vol(unsigned int value)
{
    unsigned char level;
    ...
    ext_lcd_bias_write_byte(LCD_BIAS_APPS_ADDR, NEG_OUTPUT_APPS);//change max NEG output from default 40mA to 80mA
}

static void ext_lcd_bias_set_vspn_en(unsigned int en, unsigned int seq)
{
#ifdef BUILD_LK
...
{
    ext_lcd_bias_set_vspn_en(en, seq);

    if (en) {
        ext_lcd_bias_set_vspn_vol(value);
    }
}
...
{
    if (en) {
        pmi_lcd_bias_set_vspn_vol(value);
    }

    pmi_lcd_bias_set_vspn_en(en, seq);
}
#endif
...
printk("PXL lcd_bias_dts_init  +++\n");
    /* retrieve */
    pctrl = devm_pinctrl_get(&pdev->dev);
    if (IS_ERR(pctrl)) {
        dev_err(&pdev->dev, "Cannot find disp pinctrl!");
printk("PXL lcd_bias_dts_init error!\n");
        ret = PTR_ERR(pctrl);
        goto exit;
    }
