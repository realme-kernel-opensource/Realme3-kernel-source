...
//ifdef ODM_HQ_EDIT
/* zhitongze@ODM.HQ.Multimedia.LCM 2018/10/22 modified for 2048 steps backlight */
static atomic_t g_pwm_max_backlight[PWM_TOTAL_MODULE_NUM] = { ATOMIC_INIT(2047), ATOMIC_INIT(2047) };
//#else
...
int disp_pwm_set_backlight_cmdq(enum disp_pwm_id_t id, int level_1024, void *cmdq)
{
	...
	if (old_pwm != level_1024 || atomic_cmpxchg(&g_pwm_is_change_state[index], 1, 0) == 1) {
		...
		if (level_1024 > 0) {
                        //ifdef ODM_HQ_EDIT
                        /* zhitongze@ODM.HQ.Multimedia.LCM 2018/10/22 modified for 2048 steps backlight */
			DISP_REG_MASK(cmdq, reg_base + DISP_PWM_CON_1_OFF, level_1024 << 16, 0x3fff << 16);
                        //#else
			...
			/* Avoid to set 0 */
                        //ifdef ODM_HQ_EDIT
                        /* zhitongze@ODM.HQ.Multimedia.LCM 2018/10/22 modified for 2048 steps backlight */
			DISP_REG_MASK(cmdq, reg_base + DISP_PWM_CON_1_OFF, 1 << 16, 0x3fff << 16);
                        //#else
