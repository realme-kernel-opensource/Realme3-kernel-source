...
#ifdef ODM_HQ_EDIT
/* liyan@ODM.Multimedia.LCD  2018/10/02 add for LCD bias setting */
...
int mipi_clk_change(int msg, int en)
{
	...
	*/
	...
	if (en) {
		...
		/*Yongpeng.Yi@PSW.MM.Display.LCD.Stability, 2018/06/25, add for 18531 mipi clk change 514.5->529m*/
		if (get_Operator_Version() == 25 ) {
		    def_data_rate = 1058;
		    def_dsi_hbp = 0xC0; /* adaptive HBP value */
		} else if (is_project(18531)) {
		    def_data_rate = 1058;
	...
	if (_is_power_on_status(DISP_MODULE_DSI0)) {
		...
		if (get_Operator_Version() == 25 || is_project(185311)) {
			if ((DSI_REG[0]->DSI_MODE_CTRL.MODE) != 0) {
...
{
//#ifdef ODM_HQ_EDIT
/* fanxiongnan@ODM.HQ.Multimedia.LCM 2018/10/23 added for lcm reset pin to gpio control */
#if 0
//#else
//#if 1
//#endif /* ODM_HQ_EDIT */
